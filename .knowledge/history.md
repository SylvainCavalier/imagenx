...
### 2026-08-08 — Illustrations de la landing page
- [ui] Galerie de la landing refaite avec de vraies générations de l'app en mode « Même style », deux lots de 3 images au lieu de deux lots de 4 : un café parisien au crépuscule (photographie cinématographique) et une série d'affiches art déco (train sur viaduc, phare, paquebot). Les anciennes illustrations dataient d'avant la cohérence et ne démontraient rien de particulier — leurs sujets n'avaient aucun lien entre eux. La grille passe à 3 colonnes, la 1re vignette de chaque lot porte le badge « Référence » comme dans l'app, et le sous-titre explique le mécanisme. Le hero reprend automatiquement les 2 premières de chaque lot
- [chore] Galerie servie en JPEG redimensionné à 900 px (la taille réellement affichée) au lieu de PNG pleine résolution : **16 Mo → 1 Mo** sans perte visible. Les aplats du lot « affiche vintage » compressaient très mal en PNG à cause du grain de papier, à ~1,2 Mo par image. Le glob de `Landing.vue` cible désormais `*.jpg`, et les fichiers sont numérotés pour que l'image de référence reste première dans l'ordre du glob
- [chore] Enseignement de prompt pour les visuels marketing : demander une « affiche vintage » à nano-banana produit une affiche **photographiée sur fond blanc** (mockup papier avec marge), pas une illustration plein cadre. Il faut le proscrire explicitement (`full-bleed […] no white background, no paper mockup, no margin`) — à savoir avant de régénérer ces images un jour

### 2026-08-07 — Cohérence visuelle entre les images d'un batch
- [feat] Sélecteur de cohérence dans le formulaire de génération, à trois niveaux : **Aucune** (comportement historique, seul le prompt principal relie les images), **Même style** (la 1re image sert de modèle graphique, chaque scène reste différente) et **Même scène** (les images suivantes reprennent la 1re et n'en changent que le détail demandé). Le choix est mémorisé dans les prompt presets. Badge « Référence » sur la 1re vignette du batch, visible dès l'attente puisque les autres images dépendent d'elle
- [api] Second modèle d'image branché : `google/nano-banana` sur Replicate (même clé `REPLICATE_API_KEY`, même protocole que flux, $0.039/image contre $0.040). Utilisé pour tout le batch dès qu'un mode de cohérence est actif ; l'URL de l'image de référence est passée dans son paramètre `image_input`. `black-forest-labs/flux-1.1-pro` reste le moteur par défaut
- [refactor] Choix du modèle extrait d'`ImageGenerator` vers `ImageModels::FluxPro` et `ImageModels::NanoBanana` (endpoint + forme de l'input propres à chaque modèle) ; tout le HTTP, le retry 429 et le polling restent mutualisés. Ajouter un troisième moteur ne demande plus qu'une classe
- [improve] Un batch cohérent n'est plus un fan-out plat : seul l'item de référence est enfilé à la création, et `GenerateImageJob` enfile lui-même les suivants une fois qu'il a réussi (règlement dans le `ensure`, pour qu'une erreur d'enfilement ne puisse jamais marquer en échec une image générée et déjà facturée). Si la référence échoue, les items restants sont passés en échec et remboursés — remboursement protégé par un `UPDATE` conditionnel, donc insensible au rejeu. Nouveau service `BatchDispatcher`
- [improve] Le coût est inchangé quel que soit le mode (N images, N appels, N × 8 crédits) : la 1re image demandée fait office de référence, aucune image supplémentaire n'est générée. Le mode est forcé à « Aucune » pour un batch d'une seule image, la cohérence n'ayant alors pas d'objet
- [db] `coherence_mode` (string, défaut `none`) sur `generation_batches` et `prompt_presets`, `reference_image_url` sur `generation_items` — cette dernière garde la trace de ce qui a réellement été envoyé au fournisseur, les URLs Replicate étant temporaires
- [chore] Formulation des instructions de cohérence calibrée sur des générations réelles : nano-banana étant un modèle d'édition, il repeint la référence si on ne le lui interdit pas explicitement — le mode « Même style » doit donc proscrire nommément la reprise de composition, de cadrage et d'angle. En mode « Même scène », à l'inverse, le prompt envoyé est réduit au seul sous-prompt de l'image : y répéter le prompt principal revient à redemander la scène entière et contrarie l'édition
- [chore] Specs ajoutées pour `ImageGenerator` (premier spec du service, avec WebMock), les deux modes de cohérence dans le job, la cascade d'échec de la référence et l'invariance du débit ; `spec/rails_helper.rb` fournit un `REPLICATE_API_KEY` de repli, le service en instanciant un même quand la génération est stubée
- [fix] `CLAUDE.md` corrigé sur trois points obsolètes : crédits d'essai à 80 et non 20, logique de débit/remboursement/Stripe décrite comme inexistante alors qu'elle est en place, et commande d'annotation `bundle exec annotate` qui n'existe pas (la gem est `annotaterb`, la commande est `bundle exec annotaterb models`)

### 2026-07-28 — UI de recharge des crédits et menu utilisateur
- [feat] Modale de recharge des crédits (`components/CreditsModal.vue`), ouverte depuis la pastille de crédits de la navbar au lieu d'une redirection muette vers "Mon compte" : solde converti en nombre d'images générables, les 4 paliers de recharge avec leur équivalent en images, et un bloc abonnement mis en avant avec le badge "+20% de crédits" (5 € en recharge = 500 crédits contre 600 pour 5 €/mois — seul vrai différentiel de prix, les 4 paliers ponctuels étant tous à 0,01 €/crédit). Bascule automatique sur "Gérer mon abonnement" (portail Stripe) si l'utilisateur est déjà abonné
- [ui] Le solde de crédits dans la navbar devient une pastille indigo (fond, bordure, icône éclair, `tabular-nums` pour éviter les sauts de largeur) au lieu d'un texte gris illisible ; bascule en ambre sous 8 crédits, soit quand il n'y a plus de quoi générer une seule image
- [improve] Les liens "Mon compte" et "Support" quittent la barre de navigation principale pour un menu déroulant sous l'email de l'utilisateur, avec la déconnexion en bas séparée par un filet ; fermeture au clic extérieur, à la touche Échap et au changement de route
- [refactor] Constantes de crédits centralisées dans `frontend/constants/credits.js` (coût par image, paliers de recharge, crédits d'abonnement) : le coût par image était sur le point d'être dupliqué dans la navbar, la modale et la page compte, qui référençaient chacune leur propre liste de paliers

### 2026-07-28 — Catalogue Stripe live pour la mise en production
- [api] Catalogue Stripe créé sur le compte **live** : produits `ImageNX Credits` et `ImageNX Subscription`, 4 prix ponctuels (`topup_300/500/1000/2000` à 3/5/10/20 €) et l'abonnement `subscription_monthly` à 5 €/mois. Les `lookup_key` correspondent à ceux attendus par `Api::CheckoutSessionsController` ; la tâche `rails stripe:setup` reste idempotente et ne recrée rien
- [infra] Endpoint webhook live créé sur `https://imagenx.fr/webhooks/stripe` (4 events : `checkout.session.completed`, `invoice.payment_succeeded`, `customer.subscription.updated/.deleted`) et `STRIPE_WEBHOOK_SECRET` posé sur Heroku — la variable manquait, l'endpoint plantait donc en 500 sur `ENV.fetch`
- [infra] Endpoint webhook **épinglé sur l'API Stripe `2025-02-24.acacia`** : `WebhooksController` lit `invoice.subscription` et `subscription.current_period_end`, deux champs supprimés à partir de `2025-03-31.basil` (déplacés dans `invoice.parent.subscription_details.subscription` et `subscription.items.data[0].current_period_end`). Sans ce pin, les crédits mensuels d'abonnement n'auraient jamais été attribués. À dé-épingler le jour où le contrôleur sera migré
- [infra] Configuration du Stripe Customer Portal créée en live et marquée par défaut (historique de factures, changement de moyen de paiement, résiliation en fin de période, liens CGV/confidentialité) — sans elle, `Api::PortalSessionsController` échouait faute de configuration par défaut
- [chore] Aucun Payment Link créé, volontairement : le flux passe par des Checkout Sessions créées côté serveur avec `metadata[user_id]`, que le webhook relit pour créditer le bon compte. Un Payment Link, URL statique sans `user_id`, encaisserait le paiement sans jamais créditer l'utilisateur
